        ...
        <tbody>
            @forelse ($sec_crossbows as $sec_crossbow)
                ...
                <tr>
                    ...
                    <td class="col-2 text-center">
                        <input min="0" onchange="document.getElementById('sec_crossbow_btn').click();" type="number" wire:model.defer="upper_limb.{{$loop->index}}" style="width:20px" class="d-inline rounded bg-transparent text-white text-end pe-1 shadow-none border border-{{ ($sec_crossbow->upper_limb < $sec_crossbow->r_upper_limb)?"danger":"success" }}">
                        <p class="d-none d-sm-inline">/ {{ $sec_crossbow->r_upper_limb }}</p>
                    </td>
                    <td class="col-2 text-center">
                        <input min="0" onchange="document.getElementById('sec_crossbow_btn').click();" type="number" wire:model.defer="lower_limb.{{$loop->index}}" style="width:20px" class="d-inline rounded bg-transparent text-white text-end pe-1 shadow-none border border-{{ ($sec_crossbow->lower_limb < $sec_crossbow->r_lower_limb)?"danger":"success" }}">
                        <p class="d-none d-sm-inline">/ {{ $sec_crossbow->r_lower_limb }}</p>
                    </td>
                    <td class="col-2 text-center">
                        <input min="0" onchange="document.getElementById('sec_crossbow_btn').click();" type="number" wire:model.defer="string.{{$loop->index}}" style="width:20px" class="d-inline rounded bg-transparent text-white text-end pe-1 shadow-none border border-{{ ($sec_crossbow->string < $sec_crossbow->r_string)?"danger":"success" }}">
                        <p class="d-none d-sm-inline">/ {{ $sec_crossbow->r_string }}</p>
                    </td>
                    <td class="col-2 text-center">
                        <input min="0" onchange="document.getElementById('sec_crossbow_btn').click();" type="number" wire:model.defer="receiver.{{$loop->index}}" style="width:20px" class="d-inline rounded bg-transparent text-white text-end pe-1 shadow-none border border-{{ ($sec_crossbow->receiver < $sec_crossbow->r_receiver)?"danger":"success" }}">
                        <p class="d-none d-sm-inline">/ {{ $sec_crossbow->r_receiver }}</p>
                    </td>
                    <td class="col-2 text-center">
                        <input onchange="document.getElementById('sec_crossbow_btn').click();" type="checkbox" wire:model.defer="owned.{{$loop->index}}" class="form-check-input fs-5 m-0 bg-transparent border border-{{ ($sec_crossbow->owned)?'success':'danger' }}" @if($sec_crossbow->owned) checked @endif>
                    </td>
                </tr>
            @empty
            @endforelse
