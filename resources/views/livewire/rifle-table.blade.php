<form wire:submit.prevent="update_user_rifle">
    @csrf
    <input id="rifle_btn" type="submit" class="d-none" value="@lang('Update')">
        ...
        <tbody>
            @forelse ($rifles as $rifle)
                ...
                <tr>
                    ...
                    <td class="col-2 text-center">
                        <input min="0" onchange="document.getElementById('rifle_btn').click();" type="number" wire:model.defer="barrel.{{$loop->index}}" style="width:20px" class="d-inline rounded bg-transparent text-white text-end pe-1 shadow-none border border-{{ ($rifle->barrel < $rifle->r_barrel)?"danger":"success" }}">
                        <p class="d-none d-sm-inline">/ {{ $rifle->r_barrel }}</p>
                    </td>
                    <td class="col-2 text-center">
                        <input min="0" onchange="document.getElementById('rifle_btn').click();" type="number" wire:model.defer="receiver.{{$loop->index}}" style="width:20px" class="d-inline rounded bg-transparent text-white text-end pe-1 shadow-none border border-{{ ($rifle->receiver < $rifle->r_receiver)?"danger":"success" }}">
                        <p class="d-none d-sm-inline">/ {{ $rifle->r_receiver }}</p>
                    </td>
                    <td class="col-2 text-center">
                        <input min="0" onchange="document.getElementById('rifle_btn').click();" type="number" wire:model.defer="stock.{{$loop->index}}" style="width:20px" class="d-inline rounded bg-transparent text-white text-end pe-1 shadow-none border border-{{ ($rifle->stock < $rifle->r_stock)?"danger":"success" }}">
                        <p class="d-none d-sm-inline">/ {{ $rifle->r_stock }}</p>
                    </td>
                    <td class="col-2 text-center">
                        <input onchange="document.getElementById('rifle_btn').click();" type="checkbox" wire:model.defer="owned.{{$loop->index}}" class="form-check-input fs-5 m-0 bg-transparent border border-{{ ($rifle->owned)?'success':'danger' }}" @if($rifle->owned) checked @endif>
                    </td>
                </tr>
            @empty
            @endforelse
