        ...
        <tbody>
            @forelse ($ak_weapons as $ak_weapon)
                ...
                <tr>
                    ...
                    <td class="col-2 text-center">
                        <input min="0" onchange="document.getElementById('ak_weapon_btn').click();" type="number" wire:model.defer="single_weapon.{{$loop->index}}" style="width:20px" class="d-inline rounded bg-transparent text-white text-end pe-1 shadow-none border border-{{ ($ak_weapon->single_weapon < $ak_weapon->r_single_weapon)?"danger":"success" }}">
                        <p class="d-none d-sm-inline">/ {{ $ak_weapon->r_single_weapon }}</p>
                    </td>
                    <td class="col-2 text-center">
                        <input min="0" onchange="document.getElementById('ak_weapon_btn').click();" type="number" wire:model.defer="link.{{$loop->index}}" style="width:20px" class="d-inline rounded bg-transparent text-white text-end pe-1 shadow-none border border-{{ ($ak_weapon->link < $ak_weapon->r_link)?"danger":"success" }}">
                        <p class="d-none d-sm-inline">/ {{ $ak_weapon->r_link }}</p>
                    </td>
                    <td class="col-2 text-center">
                        <input onchange="document.getElementById('ak_weapon_btn').click();" type="checkbox" wire:model.defer="owned.{{$loop->index}}" class="form-check-input fs-5 m-0 bg-transparent border border-{{ ($ak_weapon->owned)?'success':'danger' }}" @if($ak_weapon->owned) checked @endif>
                    </td>
                </tr>
            @empty
            @endforelse
